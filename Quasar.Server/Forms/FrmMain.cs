            ...
            try
            {
                this.Invoke((MethodInvoker) delegate
                {
                    ...
                    this.Text = (selected > 0)
                        ? string.Format("Quasar-魔法版 telegram: ukey321  Connected: {0} [Selected: {1}]", ListenServer.ConnectedClients.Length,
                            selected)
                        : string.Format("Quasar-魔法版 telegram: ukey321  Connected: {0}", ListenServer.ConnectedClients.Length);
                });
            }
            catch (Exception)
            {
        ...
        private void ServerState(Networking.Server server, bool listening, ushort port)
        {
            try
            {
                this.Invoke((MethodInvoker) delegate
                {
                    ...
                    listenToolStripStatusLabel.Text = listening ? string.Format("正在监听端口 {0}.", port) : "没有监听端口";
                });
                UpdateWindowTitle();
            }
